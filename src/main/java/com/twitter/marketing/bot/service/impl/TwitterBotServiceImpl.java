...
public class TwitterBotServiceImpl implements TwitterBotService {
    ...
    private String computeTweetContent() {
        ...
        int gardenListSize = Constants.pazinoGardensList.size();
        String gardenURL = Constants.pazinoGardensList.get(this.getRandomNumberUsingNextInt(gardenListSize));

        // add to 'tweetSize'
        tweetSize = tweetSize + gardenURL.length() + 1;// added 1 for an empty space

        // get a random promotional message/additional info
        int realEstateMessagesSize = Constants.realEstateQuoteList.size();
        String realEstateMessages = Constants.realEstateQuoteList.get(this.getRandomNumberUsingNextInt(realEstateMessagesSize));

        // add to 'tweetSize'
        tweetSize = tweetSize + realEstateMessages.length() + 1;// added 1 for an empty space

        List<String> validHashtagsList = new ArrayList<>();
        while(tweetSize < Constants.MAX_TWEET_LENGTH - 10) {
            int hashtagsListSize = Constants.realEstateHashtagsList.size();
            String hashtag = Constants.realEstateHashtagsList.get(this.getRandomNumberUsingNextInt(hashtagsListSize));
        ...
        tweetStatus.append("\n\n");
        tweetStatus.append(realEstateMessages);
        tweetStatus.append("\n\n");
        tweetStatus.append(gardenURL);
